# 

```
vscode@5a1fd431ee16:/$  cd /home/vscode/notebooks
vscode@5a1fd431ee16:~/notebooks$ ls
Pandas01.ipynb  a.py  b.py  data.zip  eda  test.ipynb
vscode@5a1fd431ee16:~/notebooks$ unz
bash: unz: command not found
vscode@5a1fd431ee16:~/notebooks$ unzip data.zip
Archive:  data.zip
  inflating: banklist.csv            
  inflating: billboard.csv           
  inflating: concat_1.csv            
  inflating: concat_2.csv            
  inflating: concat_3.csv            
  inflating: country_timeseries.csv  
  inflating: gapminder.tsv           
  inflating: pew.csv                 
  inflating: raw_data_urls.txt       
  inflating: scientists.csv          
  inflating: survey_person.csv       
  inflating: survey_site.csv         
  inflating: survey_survey.csv       
  inflating: survey_visited.csv      
  inflating: tesla_stock_quandl.csv  
  inflating: weather.csv             
vscode@5a1fd431ee16:~/notebooks$ mkdir data
vscode@5a1fd431ee16:~/notebooks$ mv data.zip ./data
vscode@5a1fd431ee16:~/notebooks$ cd data
vscode@5a1fd431ee16:~/notebooks/data$ unzip data.zip
Archive:  data.zip
  inflating: banklist.csv            
  inflating: billboard.csv           
  inflating: concat_1.csv            
  inflating: concat_2.csv            
  inflating: concat_3.csv            
  inflating: country_timeseries.csv  
  inflating: gapminder.tsv           
  inflating: pew.csv                 
  inflating: raw_data_urls.txt       
  inflating: scientists.csv          
  inflating: survey_person.csv       
  inflating: survey_site.csv         
  inflating: survey_survey.csv       
  inflating: survey_visited.csv      
  inflating: tesla_stock_quandl.csv  
  inflating: weather.csv             
vscode@5a1fd431ee16:~/notebooks/data$ 

```